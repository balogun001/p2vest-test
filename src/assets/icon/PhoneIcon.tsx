

import React, { memo, SVGProps } from 'react';

function Phone(props: SVGProps<SVGSVGElement>) {
  return (
    <svg {...props} width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M14.1152 16C15.1545 16 16 15.1523 16 14.1102V11.5906C16 11.3193 15.827 11.0787 15.5705 10.9932L11.8097 9.73338C11.6256 9.67126 11.4244 9.69953 11.2625 9.80657L9.66354 10.8751C7.97014 10.0656 5.9608 8.0509 5.15334 6.35305L6.21906 4.74994C6.32641 4.58814 6.35342 4.38576 6.29206 4.20124L5.03552 0.430572C4.95028 0.17348 4.71037 0 4.43979 0H1.88482C0.845487 0 0 0.838845 0 1.8809C0 9.13852 6.87657 16 14.1152 16Z" fill="white"/>
    </svg>
  );
}

const PhoneIcon = memo(Phone);
export default PhoneIcon;
